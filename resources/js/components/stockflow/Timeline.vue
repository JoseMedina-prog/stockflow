...
import { Activity, CheckCircle2, Clock, FileText, Mail, MessageCircle, MessageSquare, Phone, Trash2, Users } from 'lucide-vue-next';
...
type TimelineItem = ({ kind: 'activity' } & ActivityItem) | ({ kind: 'task' } & TaskItem);
...
const colorFor = (type: string) => {
    switch (type) {
        ...
        case 'message':
            return 'bg-cyan-500/10 text-cyan-600 dark:text-cyan-400 ring-cyan-500/20';
        case 'whatsapp':
            return 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 ring-emerald-500/20';
        case 'task':
            return 'bg-violet-500/10 text-violet-600 dark:text-violet-400 ring-violet-500/20';
        default:
    ...
    <Card>
        ...
        <CardContent>
            ...
            <ol v-else class="relative space-y-4">
                <li v-for="item in sorted" :key="`${item.kind}-${item.id}`" class="group relative flex gap-3">
                            ...
                            <div class="flex items-start justify-between gap-2">
                                <div class="flex-1">
                                    <p class="text-xs text-muted-foreground">
                                        ...
                                        · {{ item.user?.name ?? '—' }} · <span class="hidden sm:inline">{{ formatDateTime(item.occurred_at) }}</span>
                                    </p>
                                    <p v-if="item.description" class="mt-1 text-sm">{{ item.description }}</p>
                                    <p
                                        v-if="item.outcome"
                                        class="mt-1 rounded-md border border-emerald-500/20 bg-emerald-500/5 p-2 text-xs text-emerald-700 dark:text-emerald-300"
                                    >
                                ...
                                <div class="flex-1">
                                    <p class="text-xs text-muted-foreground">
                                        ...
                                        <span v-if="item.due_date"
                                            >· <span class="hidden sm:inline">{{ formatDateTime(item.due_date) }}</span></span
                                        >
                                    </p>
